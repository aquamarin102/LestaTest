...
public class ExplosionTrap : MonoBehaviour
{
    ...
    private void DealDamage()
    {
        ...
        {
            if (hitCollider.CompareTag("Player"))  // Урон наносится только игрокам
            {
                // Здесь можно вызывать метод для нанесения урона игроку
                Debug.Log("Player takes damage: " + _damage);
                // Например, hitCollider.GetComponent<PlayerHealth>().TakeDamage(damage);
            }
        }
    }
}
